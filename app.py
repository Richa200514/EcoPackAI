...
def generate_charts(results):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(base_path, "cost_chart.png"))
    plt.close()

    # CO2 Impact
    plt.figure()
    plt.plot(materials, co2, marker="o")
    plt.title("CO2 Impact")
    plt.xticks(rotation=30, ha="right")
    plt.tight_layout()
    plt.savefig(os.path.join(base_path, "co2_chart.png"))
    plt.close()

    # Suitability
    plt.figure()
    plt.bar(materials, suitability)
    plt.title("Suitability Score")
    plt.xticks(rotation=30, ha="right")
    plt.tight_layout()
    plt.savefig(os.path.join(base_path, "suitability_chart.png"))
    plt.close() 
...
def download_report():
    if not latest_results:
        ...
        "message": "Please click Predict Best Materials before downloading the report."
    }), 400
    generate_charts(latest_results)
